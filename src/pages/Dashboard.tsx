...
export default function Dashboard() {
  ...
  // Pick worst ward as the "headline" ward
  const headline: WardWeather | undefined = wards.reduce<WardWeather | undefined>(
    (worst, w) => (!worst || w.riskScore > worst.riskScore ? w : worst),
    undefined
  );
  ...
  const riskBadgeStyle =
    headline?.risk === "Extreme" ? "bg-error-container text-on-error-container border-secondary-fixed"
    : headline?.risk === "High" ? "bg-[#feebc8] text-[#dd6b20] border-[#dd6b20]"
    : "bg-surface-container text-on-surface-variant border-outline-variant";

  return (
    <main className="max-w-max-width mx-auto px-margin-desktop py-lg grid grid-cols-1 md:grid-cols-12 gap-lg h-[calc(100vh-64px)]">

      {/* ── Map section (8 cols) ── */}
      <section className="md:col-span-8 bg-surface-container-lowest rounded-xl shadow-level-1 overflow-hidden relative border border-surface-variant flex flex-col">
        <RiskMap wards={wards} loading={loading} />
      </section>

      {/* ── Sidebar (4 cols) ── */}
      <section className="md:col-span-4 flex flex-col gap-lg overflow-y-auto pb-lg">
        ...
        <div className="bg-surface-container-lowest rounded-xl shadow-level-1 p-lg border-t-4 border-error relative overflow-hidden">
          ...
          {loading || !headline ? (
            ...
            <div className="text-[72px] leading-none font-bold text-error tracking-tighter relative z-10">
              {Math.floor(headline.heatIndex)}
              <span className="text-headline-lg font-headline-lg text-on-surface-variant align-top opacity-50">
                .{Math.round((headline.heatIndex % 1) * 10)}
              </span>
            </div>
          )}
          <p className="text-body-md font-body-md text-on-surface-variant mt-sm relative z-10">
            {headline ? `Highest reading — ${headline.name}.` : "Fetching live data…"}
          </p>
        </div>
              ...
              <div key={key} className="bg-surface p-sm rounded-lg border border-surface-variant flex items-center gap-sm">
                ...
                <div>
                  ...
                  {loading || !headline ? (
                    ...
                    <div className="text-headline-md font-headline-md text-primary">{fmt(headline[key] as number)}</div>
                  )}
                </div>
              </div>
            ))}
            ...
            {loading ? (
              ...
              <div className="absolute inset-0 flex items-end justify-between px-sm pb-6">
                {trendWards.map((w) => (
                  <div
                    key={w.id}
                    title={`${w.name}: ${w.heatIndex}°C (score ${w.riskScore})`}
